...
| Supabaseの実データ保存       | 保存・公開を確認済み       | ユーザー操作で公開成功。匿名APIから公開された家を1件取得。再ログイン後の取得は未確認 |
| X OAuth 2.0                  | ログイン成功を確認済み     | ユーザーの実アカウントでログイン成功。キャンセル・ログアウトと料金は未確認           |
...
- 再読み込み・再ログイン後の復元、実環境での非公開化と別ユーザーの書き込み拒否、X側の料金、Cloudflareへの公開は引き続き未確認です。
